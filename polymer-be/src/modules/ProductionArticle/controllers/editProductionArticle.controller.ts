...
export const editProductionArticleController = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    const editingProductionArticle = await ProductionArticle.findById(articleId);
    ...
    if (editingProductionArticle) {
      if (Array.isArray(steps) && steps.length > 0) {
        ...
        steps.forEach(async (step) => {
          if (!editingProductionArticle.steps.map((t) => t.toString()).includes(step)) {
            await Step.findByIdAndUpdate(step, {
